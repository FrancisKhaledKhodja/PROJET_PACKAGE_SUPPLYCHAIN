...
def get_changement_m_to_d(mvt, code_art: str, n_lot: str, n_serie: str):
    
    mvt_n_serie = mvt.filter((pl.col("code_article") == code_art)
                      & (pl.col("n_lot") == n_lot)
                      & (pl.col("n_serie") == n_serie)).sort(["date_mvt", "n_mvt"], descending=True)
    info = {"code_article": code_art, "n_lot": n_lot, "n_serie": n_serie, "date_mvt": None, "ancien_code_projet": None, "code_projet": None, "nbre_lignes_mvt": mvt_n_serie.shape[0]}
    # Y a-t-il eu un mvt en déploiement?
    set_flag_m_d = mvt_n_serie.select(pl.col("flag_stock_m_d_bis")).unique().to_series()
    if mvt_n_serie.row(0, named=True)["flag_stock_m_d"] == "D" and "M" in set_flag_m_d:
        ...
        for i, row in enumerate(mvt_n_serie.iter_rows(named=True)):
            if row["flag_stock_m_d_bis"] == "M":
                ...
                code_projet = mvt_n_serie.row(i - 1, named=True)["code_projet"]
                info = {"code_article": code_art, "n_lot": n_lot, "n_serie": n_serie, "date_mvt": date_changement, "ancien_code_projet": ancien_code_projet, "code_projet": code_projet, "nbre_lignes_mvt": mvt_n_serie.shape[0]}
                return info
    return info
